...
//        decorChild.setBackgroundColor(Color.TRANSPARENT);//这样设置的话布局必须设置颜色，不然会出现黑色
     ...
     */
    public void bind() {
        LogUtils.e("zh","bind  SlideFinishManager.mSlideFinishLayoutList.size:: " + SlideFinishManager.getInstance().mActivityStack.size());
        if(finishAnim && mPreviousChild != null){
            LogUtils.e("zh","bind  mPreviousChild != null ");
    ...
    public void startFinishAnim() {
        LogUtils.e("zh","startFinishAnim  SlideFinishManager.mSlideFinishLayoutList.size:: " + SlideFinishManager.getInstance().mActivityStack.size());
        if(finishAnim &&  mPreviousChild != null){
